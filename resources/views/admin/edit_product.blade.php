                    ...
                    @foreach ($edit_product as $key => $product)
                        <form action="{{URL::to('/update-product/'.$product->product_id)}}" method="post" enctype='multipart/form-data'>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" value="{{ $product->product_name }}" class="form-control" name="product_name" placeholder="">
                            </div>
                            <div class="mb-3">
                                <label  class="form-label"><b>Hình ảnh sản phẩm</b></label>
                                <input type="file"  class="form-control" name="product_image" placeholder="">
                                <br>
                                <img src="{{URL::to('public/uploads/product/'.$product->product_image)}}" height="100" width="100" alt="">
                            </div>
                            <div class="mb-3">
                                <label  class="form-label"><b>Giá sản phẩm</b></label>
                                <input type="text" value="{{ $product->product_price }}" class="form-control" name="product_price" placeholder="">
                            </div>
                            <div class="mb-3">
                                <label  class="form-label"><b>Mô tả</b></label>
                                <textarea class="form-control"  style="resize:none;" name="product_desc" rows="3">{{$product->product_desc}}</textarea>
                            </div>
                        <div class="mb-3">
